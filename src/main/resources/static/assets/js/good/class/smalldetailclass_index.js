layui.use([ 'element', 'form' ], function() {
    ...
    var config = {
        url : "class/querySmallDetailTypeList",// 获取数据的url
        page : true,
        select : true,
        width:1200,
        columns : [  {
    ...
    // 添加用户方法
    $('#smallDetailClassAdd').click(function() {
        layer.open({
            type : 2,
            move : false,
            title : '添加类别',
    ...
    // 删除用户方法
    $('#smallDetailClassDel').click(function() {
        ...
        layer.confirm('是否确定删除该数据？', {
            ...
            $.post("/class/smallDetailClassDel", {
                bDSid:bDSid
            }, function(result) {
                if (!result.status) {
