...
function edit_menu(ops) {
	...
	$.post("/system/getNodeInfo.shtml", {id : node_.id},
	function(result) {
		if (!result.status) {
			layer.msg(result.message, {
				icon : 5,
...
function delete_menu(ops) {
	...
	$.post("/system/delNodeInfo.shtml", {id : node_.id},
	function(result) {
		if (!result.status) {
			layer.msg(result.message, {
				icon : 5,
...
function reload(){
	$.post("/system/loadTreeInfo.shtml",
		function(result) {
			var ops = eval($('#option').val());
			options = {
				elem : '#tree',
